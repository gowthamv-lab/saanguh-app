    ...
    play(song, queue = null, index = -1) {
        ...
        this.audio.play().catch(err => {
            ...
            if (song.id.startsWith('yt_')) {
                App.showToast('YouTube stream failed. Re-trying with new link...');
                // One-time re-try for YouTube (clear cache might be needed)
                this.play(song, queue, index);
            } else {
                App.showToast('Unable to play this song: ' + (err.message || 'Network error'));
            }
        });
